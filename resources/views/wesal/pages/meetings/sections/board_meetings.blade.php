<div class="content-card">
    <div class="page-header">
        <h1 class="page-title"><i class="fas fa-gavel" style="color: var(--primary-color);"></i> اجتماعات المجلس</h1>
    </div>
    ...
    <div style="background: rgba(255,255,255,0.05); padding: 1.5rem; border-radius: 12px; border: 2px solid var(--border-color); margin-bottom: 2rem;">
        ...
        <form method="POST" action="{{ route('wesal.meetings.board-meetings.store') }}" id="board-meeting-form" style="display: flex; flex-direction: column; gap: 1rem;">
            @csrf
            ...
            <div><label class="form-label">ملاحظات</label><textarea name="notes" class="form-control" rows="1"></textarea></div>
    ...
    @if(isset($boardMeetings) && $boardMeetings->count() > 0)
        <div class="table-container">
            <table style="direction: rtl;"><thead><tr><th>رقم</th><th>العنوان</th><th>التاريخ</th><th>المكان</th><th>الحالة</th><th>الحضور</th><th style="text-align: center;">الإجراءات</th></tr></thead><tbody>
                @foreach($boardMeetings as $bm)
                <tr><td>{{ $bm->meeting_no }}</td><td>{{ $bm->title }}</td><td>{{ $bm->meeting_date?->format('Y-m-d') }}</td><td>{{ $bm->location ?? '-' }}</td><td>{{ $bm->status === 'held' ? 'منعقد' : ($bm->status === 'postponed' ? 'مؤجل' : ($bm->status === 'cancelled' ? 'ملغي' : 'مجدول')) }}</td><td>{{ $bm->boardMembers->count() }}</td><td style="text-align: center;"><a href="{{ route('wesal.meetings.show', ['section' => 'edit-board-meeting', 'sub' => $bm->id]) }}" class="btn btn-secondary" style="padding: 0.4rem 0.8rem; margin-left: 0.25rem;"><i class="fas fa-edit"></i></a><form method="POST" action="{{ route('wesal.meetings.board-meetings.destroy', $bm) }}" style="display: inline;" onsubmit="return confirm('حذف الاجتماع؟');">@csrf @method('DELETE')<button type="submit" class="btn btn-danger" style="padding: 0.4rem 0.8rem;"><i class="fas fa-trash"></i></button></form></td></tr>
                @endforeach
            </tbody></table>
        </div>
        {{ $boardMeetings->links() }}
...
(function() {
    ...
    var initialIds = @json(old('attendee_ids', []));
    ...
    var container = document.querySelector('#board-meeting-form .mail-multiselect');
