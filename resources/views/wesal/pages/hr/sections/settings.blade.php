<div class="content-card">
    <div class="page-header">
        <h1 class="page-title">
            <i class="fas fa-cog" style="color: var(--primary-color); margin-left: 0.5rem;"></i>
            الإعدادات
        </h1>
        <p class="page-subtitle">إعدادات قسم الموارد البشرية</p>
    </div>
    <div style="background: rgba(255,255,255,0.05); padding: 2rem; border-radius: 12px; border: 2px solid var(--border-color);">
        <p style="color: var(--text-secondary); margin: 0;">إعدادات أنواع الإجازات الافتراضية، أيام العمل، الترقيم التلقائي لرقم الموظف، إلخ.</p>
    </div>
</div>
