<div class="content-card">
    <div class="page-header">
        ...
        <p class="page-subtitle">تقديم طلب عام — خاص بك فقط</p>
    </div>
    ...
    @if(!$employee)
        <div class="alert" style="background: rgba(255,152,0,0.2); border: 1px solid rgba(255,152,0,0.5); color: #fff;">
            <i class="fas fa-info-circle"></i> يجب ربط حسابك بموظف في قسم الموارد البشرية لتمكين الطلبات. تواصل مع المدير.
        </div>
    @else
        <div style="background: rgba(255,255,255,0.05); padding: 1.5rem; border-radius: 12px; margin-bottom: 2rem; border: 2px solid var(--border-color);">
            <h3 style="color: var(--text-primary); margin-bottom: 1rem;">تقديم طلب عام جديد</h3>
            <form method="POST" action="{{ route('wesal.requests.general.store') }}" style="max-width: 600px;">
                @csrf
                <div style="display: grid; gap: 1rem;">
                    <div><label class="form-label">عنوان الطلب <span style="color: #ff8a80;">*</span></label><input type="text" name="title" class="form-control" value="{{ old('title') }}" required maxlength="255" placeholder="مثال: طلب نقل ملكية"></div>
                    <div><label class="form-label">تفاصيل</label><textarea name="body" class="form-control" rows="4" placeholder="اشرح تفاصيل الطلب">{{ old('body') }}</textarea></div>
                    <div><button type="submit" class="btn btn-primary"><i class="fas fa-paper-plane"></i> تقديم الطلب</button></div>
                </div>
            </form>
        </div>

        <h3 style="color: var(--text-primary); margin-bottom: 1rem;">طلباتي العامة</h3>
        ...
        @else
            <p style="text-align: center; color: var(--text-secondary); padding: 2rem;">لا توجد طلبات عامة.</p>
        @endif
    @endif
</div>
