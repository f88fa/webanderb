...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        if (Schema::hasTable('section_order')) {
            Schema::table('section_order', function (Blueprint $table) {
                // Add columns if they don't exist
                if (!Schema::hasColumn('section_order', 'section_key')) {
                    $table->string('section_key')->unique()->after('id');
                }
                if (!Schema::hasColumn('section_order', 'section_name')) {
                    $table->string('section_name')->after('section_key');
                }
                if (!Schema::hasColumn('section_order', 'order')) {
                    $table->integer('order')->default(0)->after('section_name');
                }
                if (!Schema::hasColumn('section_order', 'is_visible')) {
