<div class="content-card">
    ...
    <div class="page-header" style="margin-bottom: 1.5rem; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 1rem;">
        <div>
            ...
            <p class="page-subtitle">سجل الخطابات والمراسلات الواردة إلى المنظمة</p>
        </div>
        <a href="{{ route('wesal.communications.incoming.create') }}" style="display: inline-flex; align-items: center; gap: 0.5rem; padding: 0.6rem 1.25rem; background: var(--primary-color); color: white; border-radius: 8px; text-decoration: none; font-weight: 600;">
            <i class="fas fa-plus"></i> تسجيل خطاب وارد
        </a>
    </div>

    <div style="display: flex; gap: 0.5rem; margin-bottom: 1rem;">
        <a href="{{ route('wesal.communications.outgoing') }}" style="padding: 0.5rem 1rem; background: rgba(255,255,255,0.1); color: var(--text-primary); border: 1px solid var(--border-color); border-radius: 6px; text-decoration: none; font-size: 0.9rem;">الصادر</a>
        <a href="{{ route('wesal.communications.incoming') }}" style="padding: 0.5rem 1rem; background: var(--primary-color); color: white; border-radius: 6px; text-decoration: none; font-size: 0.9rem;">الوارد</a>
    </div>
        ...
        <table style="width: 100%; border-collapse: collapse; direction: rtl;">
            <thead>
                <tr style="background: rgba(0,0,0,0.2);">
                    ...
                    <th style="padding: 0.75rem; text-align: right; color: var(--text-primary); font-weight: 600;">من</th>
            ...
            <tbody>
                @forelse(($letters ?? []) as $letter)
                    <tr style="border-bottom: 1px solid var(--border-color);">
                        ...
                        <td style="padding: 0.75rem; color: var(--text-primary);">{{ $letter->from_party ?: '—' }}</td>
                ...
                @empty
                    <tr>
                        <td colspan="4" style="padding: 2rem; text-align: center; color: var(--text-secondary);">لا توجد خطابات واردة. يمكنك تسجيل خطاب وارد لاحقاً.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
